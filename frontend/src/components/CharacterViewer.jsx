  ...
  return (
    <div style={{ margin: '8px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#888', marginBottom: 3 }}>
        <span>성장 XP {xp.toLocaleString()}</span>
        <span>다음 레벨까지 {(xpToNext - xp).toLocaleString()} XP</span>
      </div>
      <div style={{ background: '#e9ecef', borderRadius: 8, height: 12, overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', background: 'linear-gradient(90deg,#4caf50,#81c784)', borderRadius: 8, transition: 'width 0.8s ease' }} />
      </div>
    </div>
  );
}

export default function CharacterViewer({ character, onRefresh, onFeed }) {
  ...
  if (!character) return <div style={card}><p style={{ textAlign: 'center', color: '#999' }}>로딩 중...</p></div>;
  ...
  const balance = character.xp_balance || 0;
  const canFeed = balance >= 100;

  return (
    <div style={card}>
      <div style={{ background: bgColor, borderRadius: 16, padding: '24px 0', textAlign: 'center', marginBottom: 16, border: `2px solid ${info.color}`, position: 'relative', transition: 'background 0.5s' }}>
        {animating && <div style={levelUpBanner}>LEVEL UP! 🎉</div>}
        <div style={{ fontSize: 72, lineHeight: 1 }}>{hat ? HAT_EMOJI[hat] : ''}{info.emoji}</div>
        <div style={{ fontSize: 20, fontWeight: 700, marginTop: 8 }}>{character.username}님의 캐릭터</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        ...
        <Badge label="누적 기부" value={`${(character.total_weight_grams||0).toLocaleString()}g`} bg="#e8f5e9" />
      </div>

      <XpBar xp={character.xp} xpToNext={character.xp_to_next_level} />

      {/* XP 잔액 & 먹이주기 */}
      <div style={{ background: canFeed ? '#e8f5e9' : '#f5f5f5', borderRadius: 10, padding: '12px 16px', marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 12, color: '#888' }}>보유 XP 잔액</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: canFeed ? '#2e7d32' : '#999' }}>{balance.toLocaleString()} XP</div>
          <div style={{ fontSize: 11, color: '#aaa' }}>100 XP = 먹이 1회</div>
        </div>
        <button
          onClick={handleFeed}
          disabled={!canFeed || feeding}
          style={{
            padding: '10px 20px', borderRadius: 10, border: 'none', cursor: canFeed ? 'pointer' : 'not-allowed',
            background: canFeed ? 'linear-gradient(135deg,#66bb6a,#43a047)' : '#e0e0e0',
            color: canFeed ? '#fff' : '#aaa', fontWeight: 700, fontSize: 15,
          }}
        >
          {feeding ? '⏳' : '🍬 먹이주기'}
        </button>
...
const levelUpBanner = { position: 'absolute', top: 0, left: 0, right: 0, background: 'linear-gradient(90deg,#ff6b6b,#ffd93d)', color: '#fff', fontWeight: 700, fontSize: 16, padding: '6px 0', borderRadius: '14px 14px 0 0' };
