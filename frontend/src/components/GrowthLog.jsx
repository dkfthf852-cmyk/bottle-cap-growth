...
export default function GrowthLog() {
  ...
  if (!log.length) return <div style={card}><p style={{ color: '#999', textAlign: 'center' }}>아직 기부 기록이 없어요. 첫 기부를 해보세요! 🌱</p></div>;
        ...
        {log.map((ev) => (
          <div key={ev.id} style={row}>
            <div style={{ fontSize: 24 }}>
              {STAGE_EMOJI[ev.stage_after] || '🌱'}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600 }}>
                병뚜껑 {ev.quantity}개 기부 → <span style={{ color: '#4caf50' }}>+{ev.xp_gained} XP</span>
              </div>
              <div style={{ fontSize: 12, color: '#888' }}>
                Lv.{ev.level_before} → Lv.{ev.level_after}
                {ev.stage_changed && (
                  <span style={{ marginLeft: 6, color: '#9c27b0', fontWeight: 600 }}>
                    스테이지 업! {ev.stage_before} → {ev.stage_after}
                  </span>
                )}
                {ev.leveled_up && !ev.stage_changed && (
                  <span style={{ marginLeft: 6, color: '#1976d2' }}>레벨업!</span>
                )}
              </div>
            </div>
